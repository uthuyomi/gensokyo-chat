...
export default function EntryPageHeader() {
  ...
  const copy = useMemo(
    () => ({
      ...
      account: lang === "ja" ? "???????" : "Account",
      chat: lang === "ja" ? "?????" : "Go to chat",
      login: lang === "ja" ? "????" : "Sign in",
      guest: lang === "ja" ? "?????" : "Not signed in",
    }),
    [lang],
  );
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        {checked && user ? (
          <div className="flex min-w-0 items-center justify-between gap-3 rounded-2xl border border-border/60 bg-card/75 px-3 py-2 md:min-w-[22rem] md:justify-end">
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex items-center gap-2 text-sm text-foreground">
                ...
                <span className="truncate">{user.email ?? copy.guest}</span>
              </div>
            </div>
          ...
          <Link
            ...
            className="inline-flex shrink-0 items-center gap-2 self-start rounded-2xl border border-border bg-card px-3 py-2 text-sm font-medium text-foreground transition hover:bg-accent hover:text-accent-foreground md:self-auto"
          >
